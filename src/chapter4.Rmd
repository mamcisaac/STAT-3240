...
```{r,include=TRUE}
(bonf_ci=predict(clothing_model, newdata=data.frame(income=c(40000, 50000, 60000)), interval="confidence", level=1-0.1/3))
(WH_ci=api2lm::predict_adjust(clothing_model, newdata=data.frame(income=c(40000, 50000, 60000)), interval="confidence", method="wh", level=.9))
```
...
bonf_ci2 = api2lm::predict_adjust(clothing_model, newdata=data.frame(income=c(40000, 50000, 60000)), interval="prediction", method="bonferroni", level=.9); bonf_ci2
```

---

```{r}
scheffe_ci = bonf_ci2 = api2lm::predict_adjust(clothing_model, newdata=data.frame(income=c(40000, 50000, 60000)), interval="prediction", method="scheffe", level=.9); scheffe_ci

S = sqrt(3*qf(1-.1, 3, clothing_model$df.residual))
B = qt(1-.1/(2*3), clothing_model$df.residual)
c(S, B, S/B); 
...
(api2lm::predict_adjust(mod_physician_pop, newdata=data.frame(population=c(200000, 400000, 4000000, 6000000, 8000000)), interval="prediction", method="scheffe", level=.95))
```
