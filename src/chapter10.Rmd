...
*  The Best model for our exploratory regression analysis would be a model with personal care expenditure, transportation expenditure, recreational expenditure and total consumption expenditure as predictors. This is because the line of best fit in their respective added variable plots has a slope that is much different than zero, indicating that adding these variables to a model with all other variables will significantly reduced our sum of squares error.

* Based on the P-Values, we should include: income, marital_status, age_group, sex, food_expenditure, transportation_expenditure, and personal_care_expenditure, which indicates that they explain a lot in the prediction of clothing expenditure. Additionally, the Variance Inflation Factor for the previously mentioned predictor variables are all relatively low, which indicates that these variables do not show any severe multicollinearity problems.

---

* **Based on the given output, what is the best model for our exploratory regression analysis? Justify your reasoning.**

* Based on output, we should include all the predictor variables except the total_consumption_expenditure and total_expenditure for the best model for our exploratory regression analysis. This is because these two variables have high VIF values and that might lead to multicollinearity issues that could affect the model.
 
* There seem to be some multicollinearity between total consumption expenditure and total expenditure, so removing one of them would be the best model 
...
* I do not suspect there are any extreme outliers influencing our model, due to the fact that the absolute value of our DFBETAS never exceeds 2/sqrt(n), our DFFITS never exceeds 2/sqrt(p/n) and our cooks distance plot never comes close to the 50th percentile of F(p, n-p)

* Yes I believe there are outliers that might influence our regression model. Any points far away from 0 using the dffits, cook's d chart are potential outliers
...
* 251, 557, and 1503 might the outliers that might be unduly influencing our regression model as they seem to have high leverage values.

* points 139, 251, 1503 are notable outliers from the diagnostic plot that maybe problematic for our model
...
| Cook's $D_i$ | $\displaystyle \frac{\sum_{j=1}^n (\hat Y_j - \hat Y_{j(i)})^2}{p MSE}$ | Near or exceeding the $50$th percentile of $F(p, n-p)$ (basically $D_i > 1$), though others suggest $4/n$ | 
