...
```{r, fig.height=3, fig.cap=""}
xyplot(number_physicians ~ number_hospital_beds, data=cdi, type=c("p", "smooth"))
...
clothing_band = api2lm::predict_adjust(clothing_model, newdata=data.frame(income=x_values), interval="confidence", method="wh", level=.95)
scatter.smooth(spending_subset$income, spending_subset$clothing_expenditure)
lines(x_values, clothing_band[,"lwr"], lty=2, col="blue")
lines(x_values, clothing_band[,"upr"], lty=2, col="blue")
```
...
x_values= c(5000:100000)
clothing_model_25 = lm(I(clothing_expenditure^(.25))~income, data=spending_subset)
clothing_band_25 = api2lm::predict_adjust(clothing_model_25, newdata=data.frame(income=x_values), interval="confidence", method="wh", level=.95)
scatter.smooth(spending_subset$income, I(spending_subset$clothing_expenditure^(.25)))
lines(x_values, clothing_band_25[,"lwr"], lty=2, col="blue")
lines(x_values, clothing_band_25[,"upr"], lty=2, col="blue")
```
...
mod_physician_pop = lm(number_physicians ~ population, data=cdi)
pop_band = api2lm::predict_adjust(mod_physician_pop, newdata=data.frame(population=x_values), interval="confidence", method="wh", level=.95)
scatter.smooth(cdi$population, cdi$number_physicians)
lines(x_values, pop_band[,"lwr"], lty=2, col="blue")
lines(x_values, pop_band[,"upr"], lty=2, col="blue")
```
]

.pull-right[
```{r,  fig.cap="", include=TRUE, fig.height=5}
x_values= seq(10, 16, length.out=1000); cdi$logpopulation = log(cdi$population)
mod_physician_pop_log = lm(log(number_physicians) ~ logpopulation, data=cdi)
pop_band = api2lm::predict_adjust(mod_physician_pop_log, newdata=data.frame(logpopulation=x_values), interval="confidence", method="wh", level=.95)
